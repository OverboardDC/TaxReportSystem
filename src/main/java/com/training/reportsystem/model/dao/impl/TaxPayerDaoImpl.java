...
public class TaxPayerDaoImpl implements TaxPayerDao {
    ...
    public TaxPayerDaoImpl(Connection connection) {
    ...
    public TaxPayer login(String username, String password) {
        ...
        try (PreparedStatement taxPayerStatement = connection.prepareStatement(DaoUtil.getQuery(Queries.TAX_PAYER_LOGIN))) {
    ...
    public boolean isUsernameUnique(String username) {
        try (PreparedStatement preparedStatement = connection.prepareStatement(DaoUtil.getQuery(Queries.IS_TAX_PAYER_USERNAME_UNIQUE))) {
    ...
    public List<TaxPayer> findAll() {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(DaoUtil.getQuery(Queries.FIND_ALL_TAX_PAYERS))) {
    ...
    public TaxPayer getById(Long id) {
        TaxPayer taxPayer = null;
        try (PreparedStatement preparedStatement = connection.prepareStatement(DaoUtil.getQuery(Queries.GET_TAX_PAYER_BY_ID))) {
    ...
    public void create(TaxPayer taxPayer) {
        try (PreparedStatement preparedStatement = connection.prepareStatement(DaoUtil.getQuery(Queries.CREATE_TAX_PAYER))) {
    ...
    public void update(TaxPayer taxPayer) {
        try (PreparedStatement preparedStatement = connection.prepareStatement(DaoUtil.getQuery(Queries.UPDATE_TAX_PAYER))) {
    ...
    public void delete(Long id) {
        try (PreparedStatement preparedStatement = connection.prepareStatement(DaoUtil.getQuery(Queries.DELETE_TAX_PAYER))) {
            preparedStatement.setLong(1, id);
            preparedStatement.executeUpdate();
        } catch (SQLException e) {
            logger.error(LoggerMessages.SQL_EXCEPTION);
    ...
    public void assignInspector(Long taxPayer_id, Long inspectorId) {
        try (PreparedStatement preparedStatement = connection.prepareStatement(DaoUtil.getQuery(Queries.ASSIGN_INSPECTOR))) {
